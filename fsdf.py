...
def update(value):
    global bullets, enemies, player_health, is_game_over, spawn_timer, player_score, missed_bullets
    ...
    # Count missed bullets BEFORE removing them
    missed_bullets += len([b for b in bullets if abs(b[0]) >= 50 or abs(b[1]) >= 50])

    # Then remove bullets that went out of bounds
